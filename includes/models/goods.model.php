...
//        $goods_list = $index_key ? $this->db->getAllWithIndex($sql, $index_key) : $this->db->getAll($sql);
    ...
    function get_info( $id ) {
        ...
        if ( $goods ) {
            ...
            $goods['_images'] = array_values($image_mod->find([
                'conditions' => "goods_id = '$id'",
                'order'      => 'sort_order',
            ]));

            /* 店铺分类 */
            $goods['_scates'] = array_values($this->getRelatedData('belongs_to_gcategory', $id, [
    ...
    function update_collect_count( $goods_id ) {
        ...
        $model_goodsstatistics->edit($goods_id, [ 'collects' => $count ]);
    }
