        ...
        $('<input type="text">').css({border: '1px solid #ccc', width: '80%', height: '20px'})
            ...
            .keyup(function (event) {
                if (event.keyCode == 13) {
                    ...
                    $.get('index.php?app=' + app + '&act=ajax_col&ajax=1', {
                        id: s_id,
                        column: s_name,
                        value: editVal
                    }, function (data) {
...
//给需要修改的图片添加异步修改行为
            ...
            if (data === 'true') {
                if (i_src.indexOf('positive') > -1) {
                    if (i_src.indexOf('disabled') > -1) {
                        $('img[fieldid="' + i_id + '"][fieldname="' + i_name + '"]').attr({
                            'src': i_src.replace('disabled', 'enabled'),
                            'fieldvalue': i_val
                        });
                    }
                    else {
                        $('img[fieldid="' + i_id + '"][fieldname="' + i_name + '"]').attr({
                            'src': i_src.replace('enabled', 'disabled'),
                            'fieldvalue': i_val
                        });
                    }
                }
    ...
    //给列表有排序行为的列添加鼠标手型效果
    $('span[ectype="order_by"]').hover(function () {
