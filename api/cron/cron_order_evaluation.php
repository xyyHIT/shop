...
$times = time();
$normalTimes = $times - 7 * 86400;

/**
 * 买家7天未评价，自动好评
 */
$orders = $orderModel->db->getAll("select order_id,seller_id from ecm_order where status = 40 and finished_time < {$normalTimes} and evaluation_status = 0");
foreach ( $orders as $order ) {
//    Log::getLogger()->alert("ordersSql:select order_id,seller_id from ecm_order where status = 40 and finished_time < {$normalTimes} and evaluation_status = 0");
    $orderGoods = $orderGoodsModel->db->getAll("select * from ecm_order_goods where order_id = {$order['order_id']}");
    foreach ( $orderGoods as $orderGood ) {
//        Log::getLogger()->alert("orderGoodSql:select * from ecm_order_goods where order_id = {$order['order_id']}");
        $orderGoodsModel->edit($orderGood['rec_id'], [
            'evaluation'   => 5,
            'comment'      => '',
            'credit_value' => 1,
            'is_img'       => 0,
    ...
    $orderModel->edit($order['order_id'], [
        'is_reply_cpl'      => 1,
        'evaluation_status' => 1,
    ...
    // 获取商家的平均评分
    $avg = $orderModel->getRow("select AVG(desc_stars) desc_stars, AVG(logi_stars) logi_stars, AVG(serv_stars) serv_stars from ecm_order where seller_id = {$order['seller_id']} and evaluation_status = 1 ");

    /* 更新卖家平均评分 */
    $storeModel->edit($order['seller_id'], [
 ...
 *  卖家7天未回复，取消回复
 */
$orders = $orderModel->db->getAll("select order_id,seller_id from ecm_order where status = 40 and evaluation_time < {$normalTimes} and evaluation_status = 1 and is_reply_cpl = 0");
foreach ($orders as $order){
//    Log::getLogger()->alert("ordersSql2:select order_id,seller_id from ecm_order where status = 40 and evaluation_time < {$normalTimes} and evaluation_status = 1 and is_reply_cpl = 0");
    $orderGoods = $orderGoodsModel->db->getAll("select * from ecm_order_goods where order_id = {$order['order_id']}");
    foreach ( $orderGoods as $orderGood ) {
//        Log::getLogger()->alert("orderGoodSql2:select * from ecm_order_goods where order_id = {$order['order_id']}");
        if( $orderGood['is_reply'] == 0){
            $orderGoodsModel->edit($orderGood['rec_id'], [
                'is_reply'     => 1,
                'reply'        => '',
            ]);
