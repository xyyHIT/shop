...
class RecommendApp extends BackendApp
{
    ...
    function RecommendApp()
    {
        ...
        $this->_recommend_mod =& bm('recommend', [ '_store_id' => 0 ]);
    }
    ...
    function index()
    {
        ...
        $page = $this->_get_page();
        $recommends = $this->_recommend_mod->find([
        ...
        /* 导入jQuery的表单验证插件 */
        $this->import_resource([ 'script' => 'inline_edit.js' ]);
...
//        ));
    ...
    function ajax_col()
    {
        ...
        if ( in_array($column, [ 'sort_order' ]) ) {
            $data[ $column ] = $value;
            $this->_recommend_mod->createRelation('recommend_goods', $recom_id, [ $goods_id => [ 'sort_order' => $value ] ]);
            if ( !$this->_recommend_mod->has_error() ) {
                echo ecm_json_encode(true);
            }
        } else if ( in_array($column, [ 'sort', 'is_publish' ]) ) {
    ...
    public function updatePublish()
    {
        ...
        $this->_recommend_mod->edit($id, [ 'is_publish' => $is ]);
