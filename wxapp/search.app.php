    ...
    class SearchApp extends MallbaseApp {
        ...
        function ejGoods() {
            ...
            if ( isset( $param['cate_id'] ) && $param['layer'] === false ) {
                return $this->ej_json_failed(-1, Lang::get('no_such_category'));
            }
        ...
        function index() {
            ...
            if ( empty( $display_mode ) || !in_array($display_mode, [ 'list', 'squares' ]) ) {
        ...
        function groupbuy() {
            ...
            $groupbuy_list = $groupbuy_mod->find([
                'conditions' => $conditions,
            ...
            $this->assign('orders', $orders);
            // 当前位置
            $this->_curlocal([ [ 'text' => Lang::get('groupbuy') ] ]);
        ...
        function _get_goods_curlocal( $cate_id ) {
            ...
            $curlocal = [
                [ 'text' => LANG::get('all_categories'), 'url' => "javascript:dropParam('cate_id')" ],
            ];
            foreach ( $parents as $category ) {
                $curlocal[] = [ 'text' => $category['cate_name'], 'url' => "javascript:replaceParam('cate_id', '" . $category['cate_id'] . "')" ];
            }
        ...
        function _get_store_curlocal( $cate_id ) {
            ...
            $curlocal = [
                [ 'text' => LANG::get('all_categories'), 'url' => url('app=category&act=store') ],
            ];
            foreach ( $parents as $category ) {
                $curlocal[] = [ 'text' => $category['cate_name'], 'url' => url('app=search&act=store&cate_id=' . $category['cate_id']) ];
            }
            unset( $curlocal[ count($curlocal) - 1 ]['url'] );
        ...
        function _get_query_param() {
            ...
            if ( $res === null ) {
                ...
                if ( $keyword != '' ) {
                    ...
                    $tmp = str_replace([ Lang::get('comma'), Lang::get('whitespace'), ' ' ], ',', $keyword);
                    $keyword = explode(',', $tmp);
                    sort($keyword);
                    $res['keyword'] = $keyword;
                }
                ...
                // cate_id
                if ( isset( $_REQUEST['cate_id'] ) && intval($_REQUEST['cate_id']) > 0 ) {
                    $res['cate_id'] = $cate_id = intval($_REQUEST['cate_id']);
                    $gcategory_mod =& bm('gcategory');
                ...
                // region_id
                if ( isset( $_REQUEST['region_id'] ) && intval($_REQUEST['region_id']) > 0 ) {
                    $res['region_id'] = intval($_REQUEST['region_id']);
                }
        ...
        function _get_filter( $param ) {
            ...
            if ( $filters === null ) {
                ...
                if ( isset( $param['keyword'] ) ) {
                    ...
                    $filters['keyword'] = [ 'key' => 'keyword', 'name' => LANG::get('keyword'), 'value' => $keyword ];
                }
                isset( $param['brand'] ) && $filters['brand'] = [ 'key' => 'brand', 'name' => LANG::get('brand'), 'value' => $param['brand'] ];
                if ( isset( $param['region_id'] ) ) {
                    ...
                    $filters['region_id'] = [ 'key' => 'region_id', 'name' => LANG::get('region'), 'value' => $row['region_name'] ];
                }
                if ( isset( $param['price'] ) ) {
                    $min = $param['price']['min'];
                    $max = $param['price']['max'];
                    if ( $min <= 0 ) {
                        $filters['price'] = [ 'key' => 'price', 'name' => LANG::get('price'), 'value' => LANG::get('le') . ' ' . price_format($max) ];
                    } elseif ( $max <= 0 ) {
                        $filters['price'] = [ 'key' => 'price', 'name' => LANG::get('price'), 'value' => LANG::get('ge') . ' ' . price_format($min) ];
                    } else {
                        $filters['price'] = [ 'key' => 'price', 'name' => LANG::get('price'), 'value' => price_format($min) . ' - ' . price_format($max) ];
                    }
                }
            }
         ...
         *              'by_region'   => array(array('region_id' => region_id, 'region_name' => region_name, 'count' =>
         *              count))
