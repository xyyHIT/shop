...
class My_goodsApp extends StoreadminbaseApp {
    ...
    function My_goodsApp() {
        ...
        $this->_goods_mod =& bm('goods', [ '_store_id' => $this->_store_id ]);
    ...
    function add() {
        ...
        if ( !IS_POST ) {
            ...
            $goods_images = [];
            $desc_images = [];
            $uploadfiles = $this->_uploadedfile_mod->find([
            ...
            /* 商品图片批量上传器 */
            $this->assign('images_upload', $this->_build_upload([
                'obj'         => 'GOODS_SWFU',
                'belong'      => BELONG_GOODS,
                'item_id'     => 0,
    ...
    function edit() {
        ...
        if ( !IS_POST ) {
            ...
            /* 取到商品关联的图片 */
            $uploadedfiles = $this->_uploadedfile_mod->find([
                'fields'     => "f.*,goods_image.*",
                'conditions' => "store_id=" . $this->_store_id . " AND belong=" . BELONG_GOODS . " AND item_id=" . $id,
                'join'       => 'belongs_to_goodsimage',
                'order'      => 'add_time ASC'
            ]);
            $default_goods_images = []; // 默认商品图片
            $other_goods_images = []; // 其他商品图片
            $desc_images = []; // 描述图片
    ...
    function spec_edit() {
        ...
        if ( !IS_POST ) {
            $goods_spec = $this->_goods_mod->findAll([
                ...
                'include'    => [ 'has_goodsspec' => [ 'order' => 'spec_id' ] ],
            ]);
            ...
            $default_spec = []; // 更新商品中默认规格的信息
            foreach ( $data as $key => $val ) {
                if ( empty( $default_spec ) ) {
                    $default_spec = [ 'price' => $val['price'] ];
                }
    ...
    function ajax_col() {
        ...
        $data = [ 'goods' => [],
                  'specs' => [],
                  'cates' => [] ];
        if ( in_array($column, [ 'goods_name', 'description', 'cate_id', 'cate_name', 'brand', 'spec_qty', 'if_show', 'closed', 'recommended' ]) ) {
        ...
        } elseif ( in_array($column, [ 'price', 'stock', 'sku' ]) ) {
            if ( $column == 'price' ) {
        ...
        if ( !IS_POST ) {
            ...
            foreach ( $records as $record ) {
                ...
                foreach ( $spec_data as $spec ) {
                    ...
                    if ( empty( $default_spec ) ) {
                        $default_spec = [ 'default_spec' => $spec_id, 'price' => $spec['price'] ];
                    }
                }

                if ( !$this->_goods_mod->edit($goods_id, array_merge($spec_name, $default_spec, [ 'spec_qty' => $spec_qty ])) ) {
    ...
    function _parse_taobao_csv( $csv_string ) {
        ...
        while ( $pos < strlen($csv_string) ) {
            ...
            if ( $status == 1 ) {
                ...
                if ( $field_status == 1 && $t == ORD_QUOTE && in_array($next, [ ORD_N, ORD_R, ORD_TAB ]) ) // 引号+换行 或 引号+\t
                {
            ...
            foreach ( $record as $k => $col ) {
                ...
                switch ( $k ) {
                    case $fields_cols['description'] :
                        $return[ $key ]['description'] = str_replace([ "\\\"\\\"", "\"\"" ], [ "\\\"", "\"" ], $col);
                        break;
    ...
    function _parse_taobao_image( $col ) {
        ...
        /* 组织成数组 */
        $temp_attr = $col ? explode(';', trim($col, ';')) : [];
    ...
    function _get_goods_info( $id = 0 ) {
        ...
        $goods_info['spec_json'] = ecm_json_encode([
    ...
    function _get_post_data( $id = 0 ) {
        ...
        return [ 'goods' => $goods, 'specs' => $specs, 'cates' => $cates, 'goods_file_id' => $goods_file_id, 'desc_file_id' => $desc_file_id ];
    }
    ...
    function _check_post_data( $data, $id = 0 ) {
        if ( !$this->_check_mgcate($data['goods']['cate_id']) ) {
            $res = Lang::get('select_leaf_category');
            $this->ej_json_failed(-1, $res);
            exit( 0 );
        }
        if ( !$this->_goods_mod->unique(trim($data['goods']['goods_name']), $id) ) {
            $res = Lang::get('name_exist');
            $this->ej_json_failed(-1, $res);
            exit( 0 );
        }
        if ( $data['goods']['spec_qty'] == 1 && empty( $data['goods']['spec_name_1'] )
            ...
            $res = Lang::get('fill_spec_name');
            $this->ej_json_failed(-1, $res);
            exit( 0 );
        }
        if ( empty( $data['specs'] ) ) {
            $res = Lang::get('fill_spec');
            $this->ej_json_failed(-1, $res);
            exit( 0 );
        }
    ...
    function _save_post_data( $data, $id = 0 ) {
        ...
        if ( $id > 0 ) {
            ...
            if ( !$this->_goods_mod->edit($id, $data['goods']) ) {
                $res = Lang::get($this->_goods_mod->get_error());
                $this->ej_json_failed(-1, $res);
                exit( 0 );
            }
        ...
        if ( $id > 0 ) {
            /* 删除的规格 */
            $goods_specs = $this->_spec_mod->find([
                'conditions' => "goods_id = '{$id}'",
                'fields'     => 'spec_id'
            ]);
    ...
    function brand_apply() {
        if ( !IS_POST ) {
            ...
            if ( !$brand_id = $this->_brand_mod->add([ 'brand_name' => $brand_name, 'store_id' => $this->_store_id, 'if_show' => 0, 'tag' => trim($_POST['tag']) ]) )  //获取brand_id
            {
            ...
            $this->_brand_mod->edit($brand_id, [ 'brand_logo' => $logo ]);
            $this->pop_warning('ok',
